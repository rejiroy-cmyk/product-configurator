...
const outputPath = path.resolve(__dirname, 'mischer-accessories.json');
...
(async () => {
    ...
    if (!data.duschenmischer || !data.duschenmischer.trays) {
        console.log("No Duschenmischer found in custom-data.json");
        return;
    }

    let targetTrays = data.duschenmischer.trays;
    console.log(`Testing scraper on ${targetTrays.length} Duschenmischer items...`);
    ...
    try {
        for (let i = 0; i < targetTrays.length; i++) {
            ...
            try {
                ...
                const pdpData = await page.evaluate(() => {
                    ...
                    for (let row of accessoryRows) {
                        ...
                        if (rowText.includes('ohne abstellverschraubung') || rowText.includes('ohne verschraubung') || rowText.includes('ohne brauseschlauch') || rowText.includes('ohne handbrause')) {
                            ...
                            const isMainProduct = label.toLowerCase().includes('duschenmischer') || label.toLowerCase().includes('duschmischer');
                            if (isMainProduct) continue;
                        }

                        // Mischer Categories
                        if (rowText.includes('abstellverschraubung') || rowText.includes('verschraubung')) {
                            if (!extracted.abstellverschraubung.find(s => s.artNr === artNr)) extracted.abstellverschraubung.push({ artNr, label, type: 'Abstellverschraubung' });
                        } else if (rowText.includes('montageschiene') || rowText.includes('montageset') || rowText.includes('installationsset')) {
                            if (!extracted.montageschiene.find(s => s.artNr === artNr)) extracted.montageschiene.push({ artNr, label, type: 'Montageschiene' });
                        } else if (rowText.includes('einbaukörper') || rowText.includes('grundkörper') || rowText.includes('ibox') || rowText.includes('up-gehäuse')) {
                            if (!extracted.grundkoerper.find(s => s.artNr === artNr)) extracted.grundkoerper.push({ artNr, label, type: 'Grundkörper' });
                        } else if (rowText.includes('schlauch') && !rowText.includes('brausehalter')) {
                            if (!extracted.brauseschlauch.find(s => s.artNr === artNr)) extracted.brauseschlauch.push({ artNr, label, type: 'Brauseschlauch' });
                        } else if (rowText.includes('handbrause') || rowText.includes('brausekopf')) {
                            if (!extracted.handbrause.find(s => s.artNr === artNr)) extracted.handbrause.push({ artNr, label, type: 'Handbrause' });
                        } else if (rowText.includes('brausehalter') || rowText.includes('wandhalter')) {
                            if (!extracted.brausehalter.find(s => s.artNr === artNr)) extracted.brausehalter.push({ artNr, label, type: 'Brausehalter' });
                        } else if (rowText.includes('gleitstange') || rowText.includes('brausestange')) {
                            if (!extracted.gleitstange.find(s => s.artNr === artNr)) extracted.gleitstange.push({ artNr, label, type: 'Gleitstange' });
                        } else if (rowText.includes('anschlussbogen') || rowText.includes('wandanschluss')) {
                            if (!extracted.anschlussbogen.find(s => s.artNr === artNr)) extracted.anschlussbogen.push({ artNr, label, type: 'Anschlussbogen' });
                        }
                    }
                    return extracted;
                });
                ...
                if (pdpData && (pdpData.abstellverschraubung.length > 0 || pdpData.montageschiene.length > 0 || pdpData.grundkoerper.length > 0 || pdpData.brauseschlauch.length > 0 || pdpData.handbrause.length > 0 || pdpData.brausehalter.length > 0 || pdpData.gleitstange.length > 0 || pdpData.anschlussbogen.length > 0)) {
                    ...
                    console.log(`   -> No relevant Duschenmischer accessories found.`);
        ...
        await browser.close();
        console.log("Scraping finished. Results saved to st-scraper/mischer-accessories.json");
    }
})();
