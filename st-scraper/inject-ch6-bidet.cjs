/**
 * inject-ch6-bidet.cjs
 * Upserts all Ch6 'bidet'-classified products into custom-data.json's
 * bidet app. Each tray gets its colour/finish variants[] (InlineBOM) and
 ...
 *
 * Usage:  node inject-ch6-bidet.cjs          # DRY RUN (no writes, prints report)
 *         node inject-ch6-bidet.cjs --apply  # writes custom-data.json + prices.json
 */
'use strict';
const fs = require('fs');
...
// collect the bases: classified in ch6-api as bidet + resolved-null bases whose scraped desc classifies here
...
console.log(`=== inject-ch6-bidet ${APPLY ? '(APPLY)' : '(DRY RUN)'} ===`);
console.log(`  bidet bases considered : ${bases.size}`);
...
console.log(`  bidet total trays now  : ${trays.length}`);
if (sample) {
...
if (APPLY) {
    fs.copyFileSync(DATA, DATA + '.bak-ch6bidet');
    ...
    console.log('\nWROTE custom-data.json (backup .bak-ch6bidet) + prices.json');
} else {
