/**
 * inject-ch6-bademischer.cjs
 * Upserts all Ch6 'bademischer'-classified products into custom-data.json's
 * bademischer app. Each tray gets its colour/finish variants[] (InlineBOM) and
 ...
 *
 * Usage:  node inject-ch6-bademischer.cjs          # DRY RUN (no writes, prints report)
 *         node inject-ch6-bademischer.cjs --apply  # writes custom-data.json + prices.json
 */
'use strict';
const fs = require('fs');
...
// collect the bases: classified in ch6-api as bademischer + resolved-null bases whose scraped desc classifies here
...
console.log(`=== inject-ch6-bademischer ${APPLY ? '(APPLY)' : '(DRY RUN)'} ===`);
console.log(`  bademischer bases considered : ${bases.size}`);
...
console.log(`  bademischer total trays now  : ${trays.length}`);
if (sample) {
...
if (APPLY) {
    fs.copyFileSync(DATA, DATA + '.bak-ch6bm');
    ...
    console.log('\nWROTE custom-data.json (backup .bak-ch6bm) + prices.json');
} else {
