/**
 * inject-ch6-duschenmischer.cjs
 * Upserts all Ch6 'duschenmischer'-classified products into custom-data.json's
 * duschenmischer app. Each tray gets its colour/finish variants[] (InlineBOM) and
 ...
 *
 * Usage:  node inject-ch6-duschenmischer.cjs          # DRY RUN (no writes, prints report)
 *         node inject-ch6-duschenmischer.cjs --apply  # writes custom-data.json + prices.json
 */
'use strict';
const fs = require('fs');
...
const BRANDS = ['Alterna', 'KWC', 'Hansgrohe', 'Axor', 'Dornbracht', 'Laufen', 'Gessi', 'Similor', 'Arwa', 'Franke', 'Nussbaum', 'Neoperl', 'Keuco', 'Kludi', 'Fantini', 'MEM', 'Kartell', 'Bossini', 'Steinberg'];
const brandOf = (label) => { for (const b of BRANDS) if (new RegExp('\\b' + b + '\\b', 'i').test(label)) return b; const m = (label || '').split(/[\s,]/); return m[1] || 'Andere'; };
const leadNoun = (label) => (label || '').split(/[\s,]/)[0].replace(/[‐\-].*$/, '') || 'Zubehör';
const slug = s => String(s || 'x').toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 8) || 'x';

// collect the bases: classified in ch6-api as duschenmischer + resolved-null bases whose scraped desc classifies here
...
console.log(`=== inject-ch6-duschenmischer ${APPLY ? '(APPLY)' : '(DRY RUN)'} ===`);
console.log(`  duschenmischer bases considered : ${bases.size}`);
...
console.log(`  duschenmischer total trays now  : ${trays.length}`);
if (sample) {
...
if (APPLY) {
    fs.copyFileSync(DATA, DATA + '.bak-ch6dm');
    ...
    console.log('\nWROTE custom-data.json (backup .bak-ch6dm) + prices.json');
} else {
