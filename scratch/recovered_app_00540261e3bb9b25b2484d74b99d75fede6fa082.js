...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.customWishlist = JSON.parse(localStorage.getItem('sanitas_wishlist') || '[]');
    ...
    if (masterSearchInput && masterSearchResults) {
        ...
        masterSearchInput.addEventListener('input', (e) => {
            ...
            const regexQueries = queries.map(q => new RegExp(q.replace(/\*/g, '.*'), 'i'));
            ...
            if (limitedResults.length > 0) {
                masterSearchResults.innerHTML = limitedResults.map(r => `
                    <div class="search-result-item" onclick="handleSearchResultClick('${r.appId}', '${r.trayId || ''}', '${r.category.replace(/'/g, "\\'")}')">
                        <img src="${r.imgUrl || 'https://via.placeholder.com/32'}" class="search-result-img" onerror="this.style.display='none';">
                        <div class="search-result-info">
                            <span class="search-result-code">${r.artNr}</span>
                            <span class="search-result-desc">${r.label}</span>
                            <span class="search-result-cat">${r.category}</span>
                        </div>
                        <button class="search-result-add" title="Zur Eigenen Selektion hinzufügen" onclick="event.stopPropagation(); addMasterSearchResultToWishlist('${r.artNr}', '${r.label.replace(/'/g, "\\'")}', '${r.type}', '${r.imgUrl || ''}')">
                ...
                masterSearchResults.innerHTML = `<div style="padding: 1.5rem; text-align: center; color: var(--text-secondary);">Keine Treffer für "${query}" <br><small style="opacity:0.5;">Verwenden Sie * für Platzhalter (z.B. 1234*)</small></div>`;
                masterSearchResults.style.display = 'block';
            }
        });
        ...
        masterSearchInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                grid.innerHTML = window.currentSearchUniqueResults.map(r => `
                    <div class="product-selection-card" style="display: flex; flex-direction: column; height: 100%; border: 1px solid var(--border); box-shadow: 0 4px 6px rgba(0,0,0,0.1); cursor: default; padding: 1.5rem; border-radius: 8px;">
                        <div style="flex: 1; display:flex; align-items:center; justify-content:center; margin-bottom: 1.5rem; background:#fff; padding:1rem; border-radius:4px; max-height: 150px;">
                            <img src="${r.imgUrl || 'https://via.placeholder.com/150'}" style="max-height: 100%; max-width:100%; object-fit: contain;" onerror="this.style.display='none';">
                        </div>
                        <h4 style="font-size: 1.1rem; margin-bottom: 0.5rem;">${r.label}</h4>
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                            <span class="bom-code" style="font-size:1rem; font-weight:600; color:var(--text-primary);">${r.artNr}</span>
                            <span class="bom-type" style="background:var(--accent); color:#000; padding:2px 6px; border-radius:4px; font-size:0.8rem;">${r.type}</span>
                        </div>
                        <p style="font-size:0.85rem; color:var(--text-secondary); margin-bottom:1.5rem;">Zugehörig: ${r.category}</p>
                        <div style="margin-top:auto; display:flex; gap:0.5rem;">
                            <button class="icon-btn highlight-btn" style="flex:1; justify-content: center; background:var(--bg-surface); border:1px solid var(--accent); color:var(--text-primary);" onclick="handleSearchResultClick('${r.appId}', '${r.trayId || ''}', '${r.category.replace(/'/g, "\\'")}')">
                                Konfigurieren
                            </button>
                            <button class="icon-btn highlight-btn" style="background:var(--accent); color:#000;" title="Zur Eigenen Selektion" onclick="addMasterSearchResultToWishlist('${r.artNr}', '${r.label.replace(/'/g, "\\'")}', '${r.type}', '${r.imgUrl || ''}')">
                                <i class="ri-add-line"></i>
                            </button>
                        </div>
                    </div>
        ...
        window.handleSearchResultClick = (appId, trayId, catLabel) => {
            if (appId) {
        ...
        window.addMasterSearchResultToWishlist = (artNr, label, typ, imgUrl) => {
            ...
            feedback.innerHTML = `<i class="ri-check-line"></i> Artikel "${artNr}" zur Eigenen Selektion hinzugefügt`;
    ...
    const syncGlobalData = () => {
        const saved = localStorage.getItem('sanitas_config_data');
        if (saved) {
    ...
    if (Object.keys(bundledData).length > 0) {
        ...
        const existing = localStorage.getItem('sanitas_config_data');
        if (!existing) {
    ...
    window.renderWishlistModal = function() {
        ...
        window.customWishlist.forEach((item, index) => {
            ...
            row.innerHTML = `
                <td>
                    <i class="ri-draggable" style="cursor: grab; color: var(--text-secondary); margin-right: 0.5rem; font-size: 1.2rem;"></i>
                    <div class="img-cell" style="display: inline-flex; vertical-align: middle;">${item.imgUrl ? `<img src="${item.imgUrl}">` : `<i class="ri-image-line" style="font-size:1.2rem;color:var(--text-secondary);"></i>`}</div>
                </td>
                <td><span class="bom-code">${item.artNr}</span></td>
                <td><div class="bom-desc">${item.label}</div></td>
                <td><span class="bom-type">${item.typ || 'Artikel'}</span></td>
                <td><strong>${item.menge}</strong></td>
                <td>
                    <button class="icon-btn btn-danger" onclick="removeFromWishlist(${index})" title="Entfernen">
            ...
            wishlistTableBody.addEventListener('dragover', (e) => {
                ...
                if (tr && tr !== row && tr.dataset.index !== undefined) {
                    const rect = tr.getBoundingClientRect();
                    const midY = rect.top + rect.height / 2;
                    const draggedNode = wishlistTableBody.querySelector(`tr[data-index="${draggedRowIndex}"]`);
                    if (draggedNode && draggedNode !== tr) {
                        if (e.clientY < midY) {
                            wishlistTableBody.insertBefore(draggedNode, tr);
                        } else {
                            wishlistTableBody.insertBefore(draggedNode, tr.nextSibling);
                        }
                    }
                }
            });
    ...
    if (addAllToWishlistBtn) {
        addAllToWishlistBtn.addEventListener('click', () => {
            ...
            if (addedCount > 0) {
                ...
                feedback.style.zIndex = '9999';
                feedback.style.boxShadow = '0 4px 12px rgba(0,0,0,0.3)';
                feedback.innerHTML = `<i class="ri-check-line"></i> ${addedCount} Artikel zur Eigenen Selektion hinzugefügt`;
                document.body.appendChild(feedback);
                setTimeout(() => feedback.remove(), 3000);
            }
        });
