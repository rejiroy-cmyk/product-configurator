...
// 1. Remove the skip for Calima carriers
code = code.replace(
    "if (labelLower.includes('calima') && (cat.group === 'wannenträger' || cat.group === 'montagerahmen')) return;",
    "if (labelLower.includes('calima') && cat.group === 'montagerahmen') return; // Calima now supports carriers!"
);

// 2. Fix the Wannenträger cross-brand matching block
const oldCarrierBlock = `
...
`;
const newCarrierBlock = `
                const pMfr = (item.manufacturer || '').toLowerCase();
                // If the carrier is from a different manufacturer, it MUST explicitly mention this tray's brand or series
                if (pMfr && manufacturer && pMfr !== manufacturer) {
                    ...
                    // Fallback for generic carriers
                    return false;
                }
`;
code = code.replace(oldCarrierBlock, newCarrierBlock);

// 3. Inject Zargen Logic at the very bottom (where deduplicate/push happens)
const oldEnd = `
        // Deduplicate
...
const zargenLogic = `
        // Zargen Logic (L/U logic + 20cm margin)
        ...
        // Deduplicate
`;
code = code.replace(oldEnd, zargenLogic + `
        const uniqueArtNrs = new Set();
        matches = matches.filter(m => {
            if (!m.artNr || uniqueArtNrs.has(m.artNr)) return false;
            uniqueArtNrs.add(m.artNr);
