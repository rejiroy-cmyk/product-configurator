...
function relink() {
    console.log('--- STARTING PROTECTED SERIES RELINK (v9) ---');
    ...
    let stats = { relinked: 0, carrierFound: 0, skipped: 0 };
    ...
    if (data.duschenwanne && data.duschenwanne.trays) {
        data.duschenwanne.trays.forEach(tray => {
            ...
            // PROTECTED SERIES RULE
            const isProtected = labelLower.includes('calima') || labelLower.includes('laufen pro');
            
            // USER LOCK: Alterna ecoplan and loa are COMPLETE. Skip them.
    ...
    console.log(`\nPROTECTED SERIES RELINK COMPLETE (v9):`);
