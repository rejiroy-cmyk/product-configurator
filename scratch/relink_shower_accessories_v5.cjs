...
function relink() {
    console.log('--- STARTING PRECISION SERIES LOGIC RELINK (v5) ---');
    ...
    if (data.duschenwanne && data.duschenwanne.trays) {
        data.duschenwanne.trays.forEach(tray => {
            ...
            // 4. FRAME matching (Strict Series Logic)
            // USER RULE: Alterna ecoplan only uses Wannenträger! Only Alterna loa uses Wannenträger and Omnia frames.
            const canHaveFrame = !isEcoplan; 
    ...
    console.log(`\nPRECISION SERIES LOGIC COMPLETE (v5):`);
