...
function relink() {
    console.log('--- STARTING UNIVERSAL RULES & LOGIC RELINK (v12) ---');
    ...
    const classify = (obj) => {
        ...
        if (label.includes('träger') || label.includes('wannenträger')) return 'wannenträger';
        return 'other';
    };
    ...
    if (data.duschenwanne && data.duschenwanne.trays) {
        data.duschenwanne.trays.forEach(tray => {
            ...
            // Universal Tweak: Schaum & Schallschutz if Carrier
            if (hasCarrier) {
