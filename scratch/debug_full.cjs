...
function findMatches(cat, trayMfr, trayLabel, sizeParts) {
    ...
    return pool.filter(item => {
        ...
        if (cat.name === 'Montagerahmen') {
            if (lbl.includes('klosett') || lbl.includes('sitz') || lbl.includes('wannenfüsse') || item.artNr === '1435 191.000.000') return false;
            const isOmnia = lbl.includes('omnia');
            if (trayMfr === 'schmidlin' && !isOmnia) return false;
            if (trayMfr === 'kaldewei' && !lbl.includes('kaldewei')) return false;
            if (trayMfr === 'laufen' && !lbl.includes('ineo')) return false;
        }

        if (cat.name === 'Wannenträger') {
            const pMfr = (item.manufacturer || '').toLowerCase();
            if (pMfr && trayMfr && pMfr !== trayMfr) return false;
            if (!lbl.includes(trayMfr) && (!pMfr || pMfr === 'andere')) return false;
            if (lbl.includes('montageschaum') || lbl.includes('schallschutzset')) return false;
...
function patchTray(tray) {
    ...
    if (!hasSwissSet) {
        ...
        categories.forEach(cat => {
            ...
            if (tray.label.includes('Floor') && tray.size === '140 x 100' && cat.name === 'Montagerahmen') console.log('Matches length for Montagerahmen:', matches.length); if (matches.length > 0) {
                tray.mountingMaterials.push({
