...
function relink() {
    console.log('--- STARTING BRAND ISOLATION RELINK (v4) ---');
    ...
    if (data.duschenwanne && data.duschenwanne.trays) {
        data.duschenwanne.trays.forEach(tray => {
            ...
            // 2. SEALING TAPE
            const perimeterM = (2 * trayDims.w + 2 * trayDims.h + 200) / 1000;
            ...
            const bestTapes = tapes.filter(t => t.length >= (perimeterM - 0.5));
            ...
            // 3. CARRIER matching (Strict Brand Isolation)
            ...
            // 4. FRAME matching (Strict Brand Isolation)
            const frames = library.filter(a => {
                ...
                const isCarrier = accLabelLower.includes('träger'); // Prevent frames being marked as carriers
                ...
                // VIP Handshake: Alterna Loa -> Schmidlin Omnia
                if (isLoa) return accLabelLower.includes('omnia');
                
                // Generic rule: Don't give Omnia frames to non-Alterna/Schmidlin trays
