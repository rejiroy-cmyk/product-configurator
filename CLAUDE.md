...
- `npm test` — runs **8 suites, 175+ assertions**: `verify-duschtrennwand` (38),
  `verify-all-apps` (16), `verify-shower-rules` (10), `verify-servicepaket` (7),
  `verify-fulltext-rule` (70), `verify-product-display` (30),
  `verify-no-kitchen-in-waschtischmischer` (4), `verify-offline-fonts`. Plain Node
  with hand-rolled DOM mocks — no jsdom, no test runner. (`tests/verify-pricing.mjs`
  is the one jsdom test and is NOT in the chain; neither are the `test_*.cjs`
  scratch files.) **Run this after any change to `modules/factories/`.**
